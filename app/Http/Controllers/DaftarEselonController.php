...
class DaftarEselonController extends Controller
{
    ...
    public function index(Request $request) // <-- Terima objek Request
    {
        ...
        $allowedColumns = ['created_at', 'updated_at', 'nm_eselon'];
        if (!in_array($sortBy, $allowedColumns)) {
            $sortBy = 'created_at';
        }
        
        $eselon = Eselon::orderBy($sortBy, $sortDirection)
                            ->paginate(10)
                            ->withQueryString(); // <-- Wajib untuk mempertahankan filter saat navigasi

        return view('backend.daftar_eselon', compact('eselon', 'sortBy', 'sortDirection'));
    }
