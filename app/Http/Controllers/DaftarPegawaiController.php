...
class DaftarPegawaiController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $allowedColumns = ['created_at', 'updated_at', 'nama'];
        if (!in_array($sortBy, $allowedColumns)) {
            $sortBy = 'created_at';
        }
        
        $pegawai = Pegawai::orderBy($sortBy, $sortDirection)
                            ->paginate(10)
                            ->withQueryString(); // <-- Wajib untuk mempertahankan filter saat navigasi
        return view('backend.daftar_pegawai', compact('pegawai', 'agama',  'instansis', 'unitKerjas', 'satuanKerjas', 'sortBy', 'sortDirection'));
    }
