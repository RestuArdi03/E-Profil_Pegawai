...
class DaftarAgamaController extends Controller
{
    ...
    public function index(Request $request) // <-- Terima objek Request
    {
        ...
        $allowedColumns = ['created_at', 'updated_at', 'nm_agama'];
        if (!in_array($sortBy, $allowedColumns)) {
            $sortBy = 'created_at';
        }
        
        $agama = Agama::orderBy($sortBy, $sortDirection)
                            ->paginate(10)
                            ->withQueryString(); // <-- Wajib untuk mempertahankan filter saat navigasi

        return view('backend.daftar_agama', compact('agama', 'sortBy', 'sortDirection'));
    }
