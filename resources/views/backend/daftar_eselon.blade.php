@extends('main.layout2')@section('content')
    ...
    @php
        ...
        $currentRoute = route('backend.daftar_eselon');
    ...
    <div class="mb-4 flex justify-end items-center gap-2">
        ...
        <select id="sort_filter" onchange="window.location.href = this.value"
            ...
            <option value="{{ $currentRoute }}?sort_by=nm_eselon&direction=asc" 
                {{ $currentSortBy == 'nm_eselon' && $currentDirection == 'asc' ? 'selected' : '' }}>
                Nama Eselon (A-Z)
            </option>
            
            <option value="{{ $currentRoute }}?sort_by=nm_eselon&direction=desc" 
                {{ $currentSortBy == 'nm_eselon' && $currentDirection == 'desc' ? 'selected' : '' }}>
                Nama Eselon (Z-A)
            </option>
        ...
        <div class="overflow-x-auto">
            <div class="min-w-full inline-block align-middle">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($eselon as $esln)
                                <tr>
                                    <td class="border px-6 py-3 text-sm text-gray-800">{{ $eselon->firstItem() + $loop->iteration - 1 }}</td>
                ...
                <div class="mt-4 flex justify-end p-4">
                    {{ $eselon->links('pagination::tailwind') }}
                </div>
