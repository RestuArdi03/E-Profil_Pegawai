@extends('main.layout2')@section('content')
    ...
    @php
        ...
        $currentRoute = route('backend.daftar_strata');
    ...
    <div class="mb-4 flex justify-end items-center gap-2">
        ...
        <select id="sort_filter" onchange="window.location.href = this.value"
            ...
            <option value="{{ $currentRoute }}?sort_by=nm_strata&direction=asc" 
                {{ $currentSortBy == 'nm_strata' && $currentDirection == 'asc' ? 'selected' : '' }}>
                Strata (A-Z)
            </option>
            
            <option value="{{ $currentRoute }}?sort_by=nm_strata&direction=desc" 
                {{ $currentSortBy == 'nm_strata' && $currentDirection == 'desc' ? 'selected' : '' }}>
                Strata (Z-A)
            </option>
        ...
        <div class="overflow-x-auto">
            <div class="min-w-full inline-block align-middle">
                <div class="overflow-hidden">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($strata as $s)
                                <tr>
                                    <td class="border px-6 py-3 text-sm text-gray-800">{{ $strata->firstItem() + $loop->iteration - 1 }}</td>
                ...
                <div class="mt-4 flex justify-end p-4">
                    {{ $strata->links('pagination::tailwind') }}
                </div>
            </div>
        </div>
    </div>
